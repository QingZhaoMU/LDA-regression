...
nl <- 2000 # number of locations
nl.miss <- 2000 # number of locations with presence only data

niter <- 10000
nburn <- round(niter * .5)

nc <- 5 # true number of communities
nc.max <- 10 # maximum number of communities

nx.true <- 3 # true number of covariates
nx <- 2 # number of covariates included in analysis

ns <- 25 # number of species
nmat <- 1 # number of surveys
jump.beta <- .03

gamma <- .6 # intercept in beta regression
beta <- matrix(c(-2.15, .15, 1.25, -1.5, 0, 
                 .7, -2.35, 1.85, -.5, 0, 
                 .85, -0.8, -.85, 1.55, 0), 3, 5, byrow=T)
beta <- beta[1:nx.true, 1:nc] # slopes in beta regression
